...
class AppDelegate: UIResponder, UIApplicationDelegate {
    var window: UIWindow?
    var xGpsManager: XGPSManager = .init()
    var reservedViewController: UIViewController!
    ...
    func loadingCustomLayouts() {
        if let model = xGpsManager.currentModel {
            if model.contains(XGPSManager.XGPS150) {
                ...
                if let tabController = window?.rootViewController?.children[0] as? UITabBarController, var viewControllers = tabController.viewControllers {
                    ...
                    window?.rootViewController?.addChild(tabController)
